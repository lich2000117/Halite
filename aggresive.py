...
### This function will find nearest target and chase it
...
def agent(obs, config):
    ...
    enemy_ships = [ship for ship in opponent.ships]
    enemy_yards = [ship for ship in opponent.shipyards]
    
    # If there are no ships, use first shipyard to spawn a ship.
    if len(me.ships) < 2 and len(me.shipyards) > 0:
    ...
    for ship in me.ships:
        if ship.next_action == None:
            ...
            if point:
                ...
                # else, attack base
                point,_ = find_closest_point(ship.position, [s.position for s in enemy_yards], board)
                direction = getDirTo(ship.position, point, size)
