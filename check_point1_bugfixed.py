...
def agent(obs, config):
    ...
    def Function_A(fact_board, board, player, debug=False):
        ...
        for ship in player.ships:
            nearest_enemy_ship_old, distance_old = find_closest_point(ship.position, fact_opponent_ships, fact_board)
            nearest_enemy_ship_new, distance_new = find_closest_point(ship.position, new_opponent_ships, board)
            
            # 0. If we successfully destroyed one ship, add bonus
            if (get_ships_by_point(nearest_enemy_ship_old, fact_board) not in new_opponent_ships) and (distance_old < config["offense_distance"]):
                offense_score += get_ships_by_point(nearest_enemy_ship_old, fact_board).halite*config["attack_reward"]
                    
            # for other closest ship if not destroyed:
            if distance_new < config["offense_distance"]:
                
                # 1. Pursuit Mode if Enemy ship has less halite
                # we have less halite, attack
                halite_diff = ship.halite - get_ships_by_point(nearest_enemy_ship_new, board).halite
                if halite_diff < 0:
                    offense_score += abs(halite_diff)*(1/(1+distance_new))*config["attack_reward"]
                
                # 2. Defense Mode
                else:
                    # we have more halite, defend, move away from enemy, penalise when close
                    offense_score -= (abs(halite_diff) + 1)*(1/(1+distance_new))*config["defend_reward"]
                    ...
                    if len([True for player_yard in player.shipyards if player_yard.position == ship.position]) >= 1:
                        offense_score -= 100/(0.1+ship.halite) # if player has more halite , can defend to shipyard, otherwise penalise for going to shipyard
...
#                                 print("  B Score:", Function_B(fact_board, test_board, test_board.players[player_id], True))
    ...
    def game_update_config(board):
        ...
        if board.configuration["episodeSteps"] - cur_step < config["remain_steps_threshold"]:
            config["return_base_multiplier"] = 10
        return
